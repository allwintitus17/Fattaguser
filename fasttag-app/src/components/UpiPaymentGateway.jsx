...
const UpiPaymentGateway = ({ 
  ...
  useEffect(() => {
    console.log('Payment State:', { paymentSuccess, paymentError, currentPayment, paymentStep, isRecharge, isTollPayment });
    ...
    if (paymentSuccess && currentPayment && paymentStep === 'initiate' && !operationCompletedRef.current) {
      ...
      if (isTollPayment) {
        console.log('Toll payment processed at:', tollPlazaName);
        setPaymentStep('success');
        toast.success(`Toll payment successful at ${tollPlazaName}!`);
      } else if (isRecharge) {
        console.log('Processing recharge for tagId:', tagId);
        setPaymentStep('processing');
  ...
  }, [paymentSuccess, paymentError, currentPayment, paymentStep, dispatch, vehicleId, personalDetailsId, amount, selectedPaymentMethod, paymentMessage, isRecharge, isTollPayment, tollPlazaName, tagId]);
  ...
  const initiatePayment = () => {
    ...
    const paymentData = {
      ...
      metadata: {
        ...
        simulatedPayment: true,
        isRecharge: isRecharge,
        isTollPayment: isTollPayment,
        tollPlazaName: tollPlazaName,
        tagId: tagId || null
      }
    };

    console.log('Creating payment with flag:', paymentFlag, 'Data:', paymentData);
    dispatch(createPayment(paymentData));
  };

  const handleContinue = () => {
    if (isTollPayment && onSuccess) {
      onSuccess();
      onClose();
    } else if (isRecharge && onSuccess) {
      onSuccess();
      onClose();
    } else {
      dispatch(getUserTags());
    ...
    <div className="payment-modal-overlay" style={{
      ...
      <div className="payment-modal" style={{
        ...
        {paymentStep === 'initiate' && (
          <div style={{ textAlign: 'center' }}>
            <h2 style={{ color: '#333', marginBottom: '1rem' }}>
              {isTollPayment ? 'Toll Plaza Payment' : isRecharge ? 'Recharge FastTag' : 'Complete Payment'}
            </h2>
            ...
            <div style={{
              ...
              <div style={{ textAlign: 'left' }}>
                <p><strong>Amount:</strong> ₹{amount}</p>
                {isTollPayment && (
                  <p><strong>Toll Plaza:</strong> {tollPlazaName}</p>
                )}
                {(isRecharge || isTollPayment) && (
                  <p><strong>FastTag ID:</strong> {tagId}</p>
                )}
                <p><strong>Vehicle:</strong> {vehicleData.registrationNumber}</p>
                <p><strong>Type:</strong> {vehicleData.vehicleType}</p>
        ...
        {paymentStep === 'success' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ color: '#28a745', marginBottom: '1rem' }}>
              {isTollPayment ? 'Toll Payment Successful!' : isRecharge ? 'Recharge Successful!' : 'Payment Done!'}
            </h2>
            ...
            <p style={{ marginBottom: '1.5rem', color: '#666', fontSize: '14px' }}>
              {isTollPayment 
                ? 'Toll payment has been processed successfully. You can continue your journey!' 
                : isRecharge 
                ? 'Your FastTag has been recharged successfully!' 
                : 'Your FastTag has been created successfully and is ready to use!'}
            </p>
            ...
            >
              {isTollPayment ? 'Continue Journey →' : isRecharge ? 'Continue' : 'Go to My FastTag →'}
            </button>
          </div>
        )}
        ...
        {paymentStep === 'failed' && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ color: '#dc3545', marginBottom: '1rem' }}>
              {isTollPayment ? 'Toll Payment Failed' : isRecharge ? 'Recharge Failed' : 'Payment Failed'}
            </h2>
